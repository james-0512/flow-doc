...
處理完全部的全域前置（crosscut）封包後，額外產出一份 `crosscut-overview.md` 放在
手冊目錄——`flow-doc site` 會把它注入到「全域前置」的 index 頁，放在流程清單之前。
內容是兩張 Mermaid flowchart：

1. **路由守衛管線總覽**：把所有守衛依各敘述開頭的「執行順序（第 N 棒）」串成一條
   主線，每個節點附一句話說明；「放行、往下一棒」用實線，「中途導向」用虛線，
   所有導向匯到同一個「重新導航（整條管線從第 1 棒重跑）」節點。
2. **API 請求生命週期**：請求攔截器 → 後端 → 回應成功／錯誤兩分支，錯誤分支依
   HTTP 狀態分流，所有錯誤路徑最後匯到「錯誤仍往下拋（呼叫端可自行補償）」。
